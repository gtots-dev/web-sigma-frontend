...
interface ContractFormTypeSelectorComponentProps {
  contracts: ContractEntity[]
}

export function ContractFormTypeSelectorComponent({
  contracts
}: ContractFormTypeSelectorComponentProps) {
  const { control, watch } = useFormContext()
  const searchValue = watch('search')?.toLowerCase() || ''
  const filteredContracts = contracts.filter((op) =>
  ...
  return (
    <ul className="flex flex-col flex-1 overflow-auto">
      <Controller
        ...
        render={({ field }) => (
          <>
            {filteredContracts.map(({ id, name }) => {
              const variableRadio = `${name}-${id}`
              const contractObject = { id, name }
              const stringifiedValue = JSON.stringify(contractObject)
