  ...
  return (
    <div
      ...
      className={`flex items-center justify-between w-full transition-all duration-200 cursor-pointer group select-none relative ${
        ...
        isNested
          ? 'py-1.5 px-3 border border-zinc-200 dark:border-zinc-800 rounded bg-background/50 dark:bg-zinc-900/30 hover:border-primary-300 dark:hover:border-primary-700 hover:bg-background dark:hover:bg-zinc-900/50 text-[11px]'
          : 'py-2 px-3 border border-zinc-200 dark:border-zinc-800 rounded-md bg-background dark:bg-zinc-900/80 hover:border-primary-300 dark:hover:border-primary-700 hover:bg-zinc-50 dark:hover:bg-zinc-900 font-semibold text-xs'
      }`}
    >
      {isNested && (
        <div className="absolute -left-[10px] top-1/2 -translate-y-1/2 w-[10px] h-[1px] border border-zinc-200 dark:border-zinc-800 pointer-events-none" />
      )}
      <div className="flex items-center gap-2 min-w-0">
        <span
          className={`rounded-full shrink-0 ${
            isNested ? 'h-2 w-2' : 'h-2.5 w-2.5'
          } ${getLevelColor(data.level, data.hasData, offline)}`}
        />
        <div className="flex flex-col min-w-0">
          <span
            className={`text-foreground truncate max-w-[150px] ${isNested ? 'font-semibold text-muted-foreground group-hover:text-foreground transition-colors' : 'font-bold'}`}
          >
            {data.name}
          </span>
