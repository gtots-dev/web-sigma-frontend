...
import { MonitoringCell, MonitoringHexCell, MonitoringGridCell } from '../../domain/interfaces/monitoring-cell.interface'
...
export function useMonitoringLayout(
  ...
  const cellIdsFingerprint = useMemo(
    () => processedCells.map((i) => `${i.id}-${i.status}`).join(','),
    [processedCells]
  )

  const generateHexLinear = useCallback((width: number, r: number) => {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        list.push({ id: `${row}-${col}`, cx, cy, cell: processedCells[dataIndex] })
        dataIndex++
      }
    }
  ...
  const generateHexRadial = useCallback((width: number, height: number, r: number) => {
    const centerX = width / 2
    const list: MonitoringHexCell[] = []
    let dataIndex = 0

    if (processedCells.length > 0) {
      list.push({ id: '0-0', cx: centerX, cy: 0, cell: processedCells[dataIndex++] })
    }
    ...
    while (dataIndex < processedCells.length) {
      ...
      for (let side = 0; side < 6; side++) {
        for (let step = 0; step < ring; step++) {
          ...
          list.push({ id: `r${ring}-s${side}-p${step}`, cx: centerX + px, cy: py, cell: processedCells[dataIndex++] })
  ...
  const generateGrid = useCallback((width: number, z: number) => {
