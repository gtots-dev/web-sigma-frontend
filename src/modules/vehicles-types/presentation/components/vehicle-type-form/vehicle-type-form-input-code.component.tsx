import { HelpMeButtonComponent } from '@/modules/shared/presentation/components/help-me-button/help-me-button.component'
import {
  FormControl,
  FormField,
  FormItem,
  FormLabel,
  FormMessage
} from '@/modules/shared/presentation/components/shadcn/form'
import { Input } from '@/modules/shared/presentation/components/shadcn/input'
import type { ChangeEvent } from 'react'
import { useFormContext } from 'react-hook-form'

interface VehicleFormInputCodeComponentProps {
  require?: boolean
  description?: string
}

export function VehicleFormInputCodeComponent({
  require,
  description
}: VehicleFormInputCodeComponentProps) {
  const { control } = useFormContext()

  return (
    <FormField
      name="code"
      control={control}
      render={({ field }) => (
        <FormItem>
          <FormLabel
            className="text-sm flex items-center gap-x-1.5 dark:text-zinc-50"
            htmlFor="code-vehicle"
          >
            Código{require ? ': *' : ':'}
            <HelpMeButtonComponent description={description} />
          </FormLabel>
          <FormControl>
            <Input
              type="number"
              id="code-vehicle"
              autoComplete="off"
              min={1}
              className="!mt-1 dark:text-zinc-50 dark:border-zinc-800 focus:dark:border-zinc-50"
              placeholder="Coloque o código de identificação do tipo veículo"
              value={field.value ?? ''}
              onChange={(e: ChangeEvent<HTMLInputElement>) => {
                const value = e.target.value
                field.onChange(value === '' ? undefined : Number(value))
              }}
            />
          </FormControl>
          <FormMessage />
        </FormItem>
      )}
    />
  )
}
